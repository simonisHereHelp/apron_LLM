...
# === Paths ===
data_path = "diy_articles.jsonl"
model_path = "models/llama3-3b-instruct"
output_path = "models/llama3-RAG3b2"
...
for ex in raw_entries:
    if all(k in ex for k in ("system", "user", "assistant")):
        prompt = f"<|system|>\n{ex['system']}\n<|user|>\n{ex['user']}\n<|assistant|>\n"
        completion = ex["assistant"].strip()
        records.append({"prompt": prompt, "completion": completion})
    else:
        raise ValueError("Missing 'system', 'user', or 'assistant' field in entry.")
...
class CustomLoggingCallback(TrainerCallback):
    def __init__(self, log_file="training_log_RAG3b2.md"):
